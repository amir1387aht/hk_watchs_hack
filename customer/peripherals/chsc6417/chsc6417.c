...
void chsc6417_irq_handler(void *arg)
{
    rt_err_t ret = RT_ERROR;

    //LOG_D("chsc6417 touch_irq_handler\n");

    rt_touch_irq_pin_enable(0);

    ret = rt_sem_release(chsc6417_driver.isr_sem);
...
static rt_err_t read_point(touch_msg_t p_msg)
{
    ...
    // LOG_D("chsc6417 read_point");
    rt_touch_irq_pin_enable(1);

    {
#ifndef CHSC6417_USE_DMA_RW
        uint8_t rbuf[3] = {0};
#endif /* CHSC6417_USE_DMA_RW */
        uint8_t press = 0;

        chsc6417_i2c_read_reg8(0xE0, rbuf, 3);
        ...
        p_msg->x = (uint16_t)((((rbuf[0] & 0x40) >> 6) << 8) | rbuf[1]);
        p_msg->y = (uint16_t)(((rbuf[0] & 0x80) >> 7) << 8) | rbuf[2];
...
#ifdef CHSC6417_UPDATA_ENABLE
    extern  bool ctp_hynitron_update(void);
#endif /* CHSC6417_UPDATA_ENABLE */
...
#else
    uint8_t id = 0;
    chsc6417_i2c_read_reg8(FTS_REG_CHIP_ID, &id, 1);
    rt_kprintf("chsc6417 id=0x%x\n", id);
#endif
    /*
        {
...
{
    chsc6417_driver.probe = probe;
    chsc6417_driver.ops = &ops;
    chsc6417_driver.user_data = RT_NULL;
    chsc6417_driver.isr_sem = rt_sem_create("chsc6417", 0, RT_IPC_FLAG_FIFO);

    rt_touch_drivers_register(&chsc6417_driver);
