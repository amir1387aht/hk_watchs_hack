...
#define MAIN_APP_NAME "MainScreen"
#define MUSIC_APP_NAME "MusicScreen"
...
static void on_crown_btn_callback(button_action_t event)
{
    if (event == BUTTON_PRESSED)
    {
        if (gui_app_is_actived(MAIN_APP_NAME))
        {
            gui_app_run(MUSIC_APP_NAME);
        }
        else if (gui_app_is_actived(MUSIC_APP_NAME))
        {
            gui_app_run(MAIN_APP_NAME);
        }
        else
        {
            gui_app_goback();
...
static void app_watch_entry()
{
    ...
    int down_btn_id =
        perf_button_register(BSP_KEY1_PIN, DOWN_BUTTON_ACTIVE_POL);
    perf_button_add_trigger(down_btn_id, on_down_btn_callback);

    int crown_btn_id =
        perf_button_register(BSP_KEY2_PIN, CROWN_BUTTON_ACTIVE_POL);
